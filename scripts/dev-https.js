...
function isSupabaseRunning() {
  try {
    const result = execSync('supabase status 2>&1', { encoding: 'utf8', shell: true });
...
async function startSupabase() {
  if (!checkSupabaseInstalled()) {
    console.log('\n⚠️  Supabase CLI не установлен.');
    console.log('   Установи: brew install supabase/tap/supabase');
    return false;
  }

  if (isSupabaseRunning()) {
    console.log('✅ Supabase уже запущен');
    return true;
  }

  console.log('\n🗄️  Запускаю Supabase Local...');

  try {
    execLoud('supabase start');
    console.log('✅ Supabase Local запущен');
    console.log('   Dashboard: http://localhost:54323');
    console.log('   API: http://localhost:54321');
    return true;
  } catch (e) {
    console.error('❌ Не удалось запустить Supabase:', e.message);
    console.log('\n💡 Возможно, нужно исправить config.toml или обновить Supabase CLI:');
...
function checkPortAvailable(port, hostname = '0.0.0.0') {
...
/**
 * Находит первый свободный порт начиная с startPort
 */
async function findFreePort(startPort, hostname = '0.0.0.0', maxAttempts = 100) {
...
async function startServer({ certPath, keyPath, hostname, port, httpPort }) {
  ...
  console.log('\n' + '═'.repeat(60));
  console.log('📱 DANANG EXPAT HUB - TELEGRAM MINI APP READY');
...
(async () => {
  ...
  // Запускаем Supabase Local
  const skipSupabase = process.env.SKIP_SUPABASE === '1' || process.env.SKIP_SUPABASE === 'true';
  if (!skipSupabase) {
    await startSupabase();
  } else {
    console.log('\n⏭️  Supabase отключён (SKIP_SUPABASE=1)');
  }
  ...
  // Находим свободный порт для HTTPS (Next.js)
  const preferredPort = Number(process.env.DEV_HTTPS_PORT || 3001);
  console.log(`\n🔍 Ищу свободный порт для Next.js (начиная с ${preferredPort})...`);

  let httpsPort;
  try {
    httpsPort = await findFreePort(preferredPort, hostname);
    if (httpsPort !== preferredPort) {
      console.log(`⚠️  Порт ${preferredPort} занят, использую ${httpsPort}`);
    } else {
      console.log(`✓ Порт ${httpsPort} свободен`);
    }
  } catch (err) {
  ...
  // Находим свободный порт для HTTP (Cloudflare Tunnel)
  const httpPortPreferred = httpsPort + 1000;
  console.log(`🔍 Ищу свободный порт для HTTP tunnel (начиная с ${httpPortPreferred})...`);

  let httpPort;
  try {
    httpPort = await findFreePort(httpPortPreferred, '127.0.0.1');
    console.log(`✓ HTTP порт для tunnel: ${httpPort}`);
  } catch (err) {
    console.error('❌ Не удалось найти свободный HTTP порт:', err.message);
    process.exit(1);
  }

  try {
    await startServer({ certPath, keyPath, hostname, port: httpsPort, httpPort });
  } catch (err) {
